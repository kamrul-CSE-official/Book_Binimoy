...
const getRandomPrice = () => {
  ...
  const takaAmount = randomAmount;

  return takaAmount;
};

export default function Card({ result }) {
  const dynamicPrice = getRandomPrice();

  return (
    <div
      key={result?.key}
      className="relative m-10 w-full max-w-xs overflow-hidden rounded-lg bg-white shadow-md cursor-pointer hover:shadow-xl"
    >
        ...
        <div className="flex items-center justify-between">
          <p>
            ...
            <span className="text-sm text-slate-900 line-through">
              ৳{dynamicPrice + (dynamicPrice * 15) / 100}
            </span>
          </p>
          <a
            href="#"
